...
export const WarehouseTab: React.FC<Props> = ({
    ...
    const [selectedBalanceForLayers, setSelectedBalanceForLayers] = useState<StockBalance | null>(null);
    ...
    const balanceColumns: DataTableColumn<StockBalance>[] = [
        ...
        },
        {
            ...
            align: 'right',
            render: (b) => (
                <span className={`font-bold ${Number(b.qty_on_hand) > 0 ? 'text-emerald-700' : 'text-rose-600'}`}>
                    {b.qty_on_hand}
                </span>
            ),
        },
            ...
            render: (b) => (
                <div className="flex items-center justify-end gap-2">
                    <button
                        ...
                        className="inline-flex items-center text-xs font-semibold text-indigo-600 hover:text-indigo-900 bg-indigo-50 px-2.5 py-1 rounded-md"
                    >
                        Layer FIFO
                    </button>
                    <Link
                        href={`/warehouse/stock-movements?product_variant_id=${b.product_variant_id}&warehouse_id=${b.warehouse_id}`}
                        className="inline-flex items-center text-xs font-semibold text-slate-600 hover:text-slate-900 bg-slate-100 px-2.5 py-1 rounded-md"
                    >
                        Kartu Stok &rarr;
                    </Link>
                </div>
    ...
    const adjustmentColumns: DataTableColumn<StockAdjustment>[] = [
        {
            ...
            render: (a) => <span className="font-mono text-xs font-bold text-slate-900">{a.adjustment_number}</span>,
        },
        {
            key: 'type',
            header: 'Tipe',
    ...
    const transferColumns: DataTableColumn<StockTransfer>[] = [
        {
            key: 'id',
            header: 'ID Transfer',
            render: (t) => <span className="font-mono text-xs font-bold text-slate-900">#{t.id}</span>,
        },
        {
            key: 'from',
            header: 'Gudang Asal',
        ...
        },
        {
            ...
            render: (t) => {
                const badges: Record<string, { label: string; className: string }> = {
                    ...
                    shipped: { label: 'SHIPPED', className: 'bg-indigo-50 text-indigo-700 ring-1 ring-indigo-600/20' },
                    received: { label: 'RECEIVED', className: 'bg-emerald-50 text-emerald-700 ring-1 ring-emerald-600/20' },
                    cancelled: { label: 'CANCELLED', className: 'bg-rose-50 text-rose-700 ring-1 ring-rose-600/20' },
                };
                const info = badges[t.status] ?? { label: t.status, className: 'bg-slate-100 text-slate-700' };
                return (
        ...
        <div className="space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-2 border-b border-slate-200 pb-3">
                ...
                <Link
                    ...
                    className="ml-auto px-3 py-1.5 text-xs font-semibold rounded-lg bg-slate-100 text-slate-700 hover:bg-slate-200 transition-all flex items-center gap-1.5"
                >
            ...
            {subTab === 'balances' && (
                <div className="space-y-3">
                    ...
                    <div className="flex flex-wrap items-center gap-3 bg-slate-50 p-3 rounded-lg border border-slate-200/80">
                        <input
                            type="text"
                ...
                <div className="space-y-3">
                    <div className="flex justify-end">
                        <Link href="/warehouse/adjustments/create">
                            <Button variant="primary">+ Buat Penyesuaian Stok</Button>
                        </Link>
                    </div>
            ...
            {subTab === 'transfers' && stockTransfers && (
                <div className="space-y-3">
                    <div className="flex justify-end">
                        <Link href="/warehouse/stock-requests/create">
                            <Button variant="primary">+ Buat Transfer Stok (Request)</Button>
                        </Link>
                    </div>
                ...
                >
                    <div className="space-y-4">
                        <div className="rounded-lg bg-slate-50 p-3 text-xs text-slate-700 space-y-1">
                            <div>
                                <span className="font-semibold">Varian Produk:</span>{' '}
                                {selectedBalanceForLayers.product_variant?.product?.name} -{' '}
                                {selectedBalanceForLayers.product_variant?.variant_name}
                            </div>
                            <div>
                            ...
                            <div>
                                <span className="font-semibold">Total On Hand:</span>{' '}
                        ...
                        <div className="space-y-2">
                            ...
                            {layersLoading ? (
                                ...
                                <div className="py-6 text-center text-xs text-slate-500">
                                    Tidak ada layer FIFO aktif (stok 0 atau belum ada pergerakan layer).
                                </div>
                            ) : (
                                        ...
                                        >
                                            <div>
                                                <div className="font-bold text-slate-900">
                                                    ...
                                                    <span className="font-normal text-slate-500">
                                                        ({layer.source_type ?? 'pembelian'})
                                                    </span>
                                                </div>
                                                <div className="text-[11px] text-slate-500">
                                                    Diterima:{' '}
                                                    {layer.received_at
                                                        ? new Date(layer.received_at).toLocaleString('id-ID')
                                                        : '-'}
                                                </div>
                                            </div>
                                            ...
                                            <div className="text-right">
                                                <div className="font-bold text-indigo-600">
                                                    Rp {Number(layer.unit_cost).toLocaleString('id-ID')} / unit
                                                </div>
                                                <div className="text-slate-700 font-semibold">
                                                    Sisa Qty: {layer.qty_remaining}
                                                </div>
                                            </div>
                                        </div>
                                    ))}
                        ...
                        <div className="flex justify-end pt-3 border-t border-slate-100">
                            <Button
                                variant="secondary"
                                onClick={() => setSelectedBalanceForLayers(null)}
                            >
                                Tutup
                            </Button>
                        </div>
