...
export default function PurchaseInvoicesIndex({ purchaseInvoices }: Props) {
    ...
    const columns: DataTableColumn<PurchaseInvoice>[] = [
        ...
        },
        {
            key: 'supplier_id',
            header: 'Vendor ↕',
            render: (row) => <span className="font-medium text-indigo-600">{row.supplier_name || 'PT. Behaestex'}</span>,
        },
        {
            key: 'due_date',
            header: 'Due date ↕',
    ...
    const unpaidTotal = purchaseInvoices.data.reduce((sum, i) => sum + Number(i.total || 0), 0);

    return (
        <CompanyLayout>
            <Head title="Purchases - Invoices" />
            <div className="space-y-6">
                <PageHeader title="Purchases" actions={<PurchasingHeaderDropdown />} />

                <PurchasingSummaryCards unpaidTotal={unpaidTotal} unpaidCount={purchaseInvoices.data.length} />

                <PurchasingTabs activeTab="invoices" />
                ...
                <div className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
                    <DataTable<PurchaseInvoice>
                        ...
                        emptyMessage="Belum ada data Purchase Invoice."
                    />
                </div>
            </div>
        </CompanyLayout>
