...
export default function PurchaseQuotesIndex({ purchaseQuotes }: Props) {
    ...
    const columns: DataTableColumn<PurchaseQuote>[] = [
        {
            ...
            render: (row) =>
                row.quote_date
                    ? new Date(row.quote_date).toLocaleDateString('id-ID', {
                          day: '2-digit',
                          month: '2-digit',
                          year: 'numeric',
                      })
        ...
        },
        {
            key: 'supplier_id',
            header: 'Vendor ↕',
            render: (row) => <span className="font-medium text-indigo-600">{row.supplier_name || 'Supplier'}</span>,
        },
        {
            key: 'valid_until',
            header: 'Valid until ↕',
            render: (row) =>
                row.valid_until
                    ? new Date(row.valid_until).toLocaleDateString('id-ID', {
                          day: '2-digit',
                          month: '2-digit',
                          year: 'numeric',
                      })
        ...
        <CompanyLayout>
            ...
            <div className="space-y-6">
                ...
                <div className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
                    <DataTable<PurchaseQuote>
                        columns={columns}
                        rows={purchaseQuotes.data}
                        getRowKey={(row) => row.id}
                        emptyMessage="Belum ada data Purchase Quote."
                    />
                </div>
            </div>
        </CompanyLayout>
